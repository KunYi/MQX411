   ...
   if (if_ptr && if_ptr->ARP) {
      ...
      while (waiting_ptr) {
         ...
         if (src_addr == IP_source(waiting_ptr)) {
            ...
            hdrlen = (mqx_ntohc(iphead->VERSLEN) & 0x0F) << 2;
...
{ 
   ...
   mqx_htonc(packet->HEAD.TYPE, ICMPTYPE_ECHO_REQ);
   mqx_htonc(packet->HEAD.CODE, 0);
   mqx_htons(packet->HEAD.CHECKSUM, 0);
   mqx_htons(packet->ID,  parms->ping_param->id);
   mqx_htons(packet->SEQ, parms->seq);

   chksum = IP_Sum_PCB(0, pcb);
   chksum = IP_Sum_invert(chksum);
   mqx_htons(packet->HEAD.CHECKSUM, chksum);
   pcb->IP_SUM_PTR = NULL;
