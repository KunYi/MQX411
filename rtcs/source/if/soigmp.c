...
uint32_t SOL_IGMP_setsockopt  (uint32_t, uint32_t, uint32_t, void *, uint32_t);
...
static void IGMP_setopt (IGMP_PARM_PTR);
       ...
       uint32_t IGMP_join_if          (ip_mreq *);
       uint32_t IGMP_leave_if         (ip_mreq *);
       uint32_t IGMP_leave_if_all     (MC_MEMBER_PTR *);
static uint32_t IGMP_join_socket      (MC_MEMBER_PTR *, ip_mreq *, uint32_t (_CODE_PTR_ *)(MC_MEMBER_PTR *));
static uint32_t IGMP_leave_socket     (MC_MEMBER_PTR *, ip_mreq *, uint32_t (_CODE_PTR_ *)(MC_MEMBER_PTR *));
static uint32_t IGMP_leave_socket_all (ip_mreq *);

uint32_t IGMP_get_membership (IGMP_PARM_PTR);
...
uint32_t IGMP_join_group
   (
      MC_MEMBER_PTR         *phead,   /* [IN] head of group list */
      ip_mreq               *igrp,    /* [IN] group decriptor */
      uint32_t (_CODE_PTR_   *pfn)(MC_MEMBER_PTR *)
   )
{ /* Body */
   uint32_t  status;
