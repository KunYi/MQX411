...
{ /* Body */
   ...
   mqx_htons(packet->LINKTYPE, ARPLINK_ETHERNET);
   mqx_htons(packet->PROTTYPE, ARPPROT_IP);
   mqx_htonc(packet->LINKLEN,  6);
   mqx_htonc(packet->PROTLEN,  4);
   mqx_htons(packet->OPCODE,   ARPOP_REQUEST);
...
{ /* Body */
   ...
   */
   ...
   mqx_htonl(packet->PROTSRC,  arp_ptr->WAITADDR);
   mqx_htonl(packet->PROTDEST, arp_ptr->PADDR);
...
{ /* Body */
   ...
   if (IP_is_local(if_ptr, lpaddr) || ARP_do_proxy(if_ptr, lpaddr)) {
    ...
    if (opcode == ARPOP_REQUEST)
    {
        ...
        mqx_htons(packet->OPCODE, ARPOP_REPLY);
        htone(packet->LINKSRC,  arp_cfg->LADDR);
        mqx_htonl(packet->PROTSRC,  lpaddr);
        htone(packet->LINKDEST, rladdr);
        mqx_htonl(packet->PROTDEST, rpaddr);
