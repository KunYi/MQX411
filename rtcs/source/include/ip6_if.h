...
{
    in6_addr    dns_addr;           /* IPv6 address of the DNS Server. */
    uint32_t    creation_time;      /* Time of entry creation, in seconds.*/    
...
typedef struct ip6_if_struct
{
    ...
    IP6_DNS_ENTRY           dns_address[RTCSCFG_IP6_IF_DNS_MAX];/* DNS Server List.*/ 

    ND6_CFG_PTR             ND6;                                /* Pointer to NDP control structure.*/
...
struct ip_if *ip6_if_get_by_n( uint32_t n );
...
#endif
void IP6_IF_bind_addr  (IP6_IF_PARM_PTR);
void IP6_IF_unbind_addr (IP6_IF_PARM_PTR);
