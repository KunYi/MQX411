...
#define MEM_TYPE_FTPSRV_STRUCT                  (MEM_TYPE_RTCS_BASE+53)
...
extern uint32_t _UDP_max_queue_size;    /* maximum length of UDP queue */
...
typedef struct ft_call_struct {
   uint32_t     (_CODE_PTR_ OPEN)    (void *ft_data);
   unsigned char   *(_CODE_PTR_ READ)    (uint32_t *size);
   bool     (_CODE_PTR_ EOFT)    (void);
   uint32_t     (_CODE_PTR_ CLOSE)   (void);
} FT_CALL_STRUCT, * FT_CALL_STRUCT_PTR;
...
typedef struct tftp_data_struct {
   ...
   char         *FILENAME;
   char         *FILEMODE;
} TFTP_DATA_STRUCT, * TFTP_DATA_STRUCT_PTR;

/*
** File Format information structure
*/
typedef struct ff_call_struct {
   uint32_t     (_CODE_PTR_ START)   (void *ff_data);
   uint32_t     (_CODE_PTR_ DECODE)  (uint32_t size, unsigned char *data);
   uint32_t     (_CODE_PTR_ EOFD)    (void);
   void         (_CODE_PTR_ EXEC)    (void);
} FF_CALL_STRUCT, * FF_CALL_STRUCT_PTR;
...
} BINFILE_DATA_STRUCT, * BINFILE_DATA_STRUCT_PTR;
...
typedef struct host_entry_struct
{
   uint32_t     ip_address;
   char         *host_name;
   char         *aliases[MAX_HOST_ALIASES+1];
} HOST_ENTRY_STRUCT;
...
typedef struct internal_hostent_struct
{
   ...
   char             *ALIASES[MAX_HOST_ALIASES+1];
   uint32_t         *ADDRESSES[MAX_HOST_ADDRESSES+1];
   uint32_t         IP_address;
} INTERNAL_HOSTENT_STRUCT, * INTERNAL_HOSTENT_STRUCT_PTR;
...
typedef struct tcpip_parm {
   void         *SYNC;      /* for synchronization with application */
...
extern HOSTENT_STRUCT  *DNS_gethostbyname
(
   char       *name_ptr
);

extern HOSTENT_STRUCT  *DNS_gethostbyaddr
(
   char         *addr_ptr,
   uint32_t     len,
   uint32_t     type
...
(
   char     *hostname    /* [IN] name of host to search for */
);

#define gethostbyname   DNS_gethostbyname
#define gethostbyaddr   DNS_gethostbyaddr

#endif /* RTCSCFG_ENABLE_DNS */


extern uint32_t RTCS_load
(
   FT_CALL_STRUCT_PTR         , /* [IN] file transfer protocol to use */
   void                      *, /* [IN] file transfer data */
   FF_CALL_STRUCT_PTR         , /* [IN] file format to decode */
   void                      *, /* [IN] file format data */
   uint32_t                     /* [IN] optional flags */
);

extern uint32_t RTCS_load_TFTP_BIN
(
   _ip_address              ,   /* [IN] address of the TFTP server */
   char                     *,  /* [IN] file to retrieve */
   unsigned char            *   /* [IN] download start address */
);

extern uint32_t RTCS_exec_TFTP_BIN
(
   _ip_address              ,   /* [IN] address of the TFTP server */
   char                     *,  /* [IN] file to retrieve */
   unsigned char            *,  /* [IN] download start address */
   unsigned char            *   /* [IN] execution start address */
);

extern uint32_t RTCS_load_TFTP_SREC
(
   _ip_address              ,   /* [IN] address of the TFTP server */
   char                     *   /* [IN] file to retrieve */
);

extern uint32_t RTCS_exec_TFTP_SREC
(
   _ip_address              ,   /* [IN] address of the TFTP server */
   char                     *   /* [IN] file to retrieve */
);

extern uint32_t RTCS_load_TFTP_COFF
(
   _ip_address              ,   /* [IN] address of the TFTP server */
   char                     *   /* [IN] file to retrieve */
);

extern uint32_t RTCS_exec_TFTP_COFF
(
   _ip_address              ,  /* [IN] address of the TFTP server */
   char                     *  /* [IN] file to retrieve */
);

extern uint32_t ECHOSRV_init        (char *, uint32_t, uint32_t);
extern uint32_t TELNETSRV_run       ( RTCS_TASK_PTR );

extern uint32_t TELNETSRV_init      (char *, uint32_t, uint32_t, RTCS_TASK_PTR);
extern uint32_t TELNETSRV_sockinit  (char *, uint32_t, uint32_t, RTCS_TASK_PTR, uint32_t);
extern uint32_t DHCPSRV_init        (char *, uint32_t, uint32_t);
extern uint32_t TFTPSRV_init        (char *, uint32_t, uint32_t);
...
extern uint32_t ECHOSRV_stop       (void);
extern uint32_t SNMP_stop          (void);
extern uint32_t TFTPSRV_stop       (void);
extern uint32_t TELNETSRV_stop     (void);

extern bool TFTPSRV_access (char *, uint16_t);
...
extern uint32_t TELNET_connect   (_ip_address);
extern uint32_t SNTP_oneshot     (_ip_address, uint32_t);
extern void MIB1213_init (void);
extern void MIBMQX_init  (void);
extern void ECHO_task         (uint32_t);
extern void TELNET_server     (uint32_t);
...
#if RTCSCFG_ENABLE_DNS /* Obsolete.*/

extern uint32_t DNS_init
(
   void
);
...
extern _ip_address LWDNS_server_ipaddr; /* Obsolete.*/
