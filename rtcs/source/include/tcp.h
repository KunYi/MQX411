...
typedef struct tcp_options_struct  {
   ...
   */
   ...
   uint16_t     locport;       /* Local TCP port - use 0 to have one
                                 assigned */
   uint16_t     remport;       /* Remote port to call or wait for (may be
                                 0 if can accept from any host port) */

   int32_t      keepalive;     /* "Keepalive" timeout (0 to disable) */
...
typedef struct tcb_parm {
   ...
   _ip_address    IP_ADDR;
    ...
    uint32_t       IF_SCOPE_ID;   /* Interface number */
#endif
    uint16_t        AF;           /* Inet family      */  
   ...
   int32_t         SOFTERROR;     /* Last error code in TCP context */
...
typedef struct tcb_struct {
   ...
   uint16_t   RESERVED1655;
   uint16_t   oevents;        /* event(s) oclient is waiting for... */
#endif
   ...
   uint32_t   if_scope_id;      /* interface number, scope_id */
#endif
   uint16_t   af;               /* inet family */ 


   uint16_t   local_port;     /* local TCP port [...] */
   uint16_t   remote_port;    /* remote TCP port, may be 0 in LISTEN state */
   ...
   uint16_t   i_local_port;   /* initial local_port */
   uint16_t   i_remote_port;  /* initial remote_port */
#endif
