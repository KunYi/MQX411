...
uint32_t SREC32_start  (void *ff_data);
uint32_t SREC32_decode (uint32_t size, unsigned char *data);
uint32_t SREC32_eod    (void);
...
{ /* Body */
   ...
   while (size--) {
      ...
      case hex_d3lo:
         ...
         SREC_config.word <<= 8;
         SREC_config.word += SREC_config.data;
         *(uint32_t *)SREC_config.addr = SREC_config.word;
         SREC_config.addr++;
