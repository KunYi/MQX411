...
{ 
    ...
    filename = ftpsrv_strip_path_delimiters(session->cmd_arg);
...
int32_t ftpsrv_mkdir(FTPSRV_SESSION_STRUCT* session)
{
    ...
    ftpsrv_convert_percents(dir_name);
    path = ftpsrv_strip_path_delimiters(dir_name);
    ...
    _mem_free(full_path);
    if(error == MFS_CANNOT_CREATE_DIRECTORY)
    {
...
int32_t ftpsrv_rmdir(FTPSRV_SESSION_STRUCT* session)
{
    ...
    ftpsrv_convert_percents(session->cmd_arg);
    dir_name = ftpsrv_strip_path_delimiters(session->cmd_arg);
