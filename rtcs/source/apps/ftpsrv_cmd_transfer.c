...
int32_t ftpsrv_transfer(FTPSRV_SESSION_STRUCT* session, uint32_t mode)
{
    ...
    err_code = RTCS_task_create("ftpsrv transfer", FTPSRV_TRANSFER_TASK_PRIO, FTPSRV_SESSION_STACK_SIZE, ftpsrv_transfer_task, &trans_param);
    if (MQX_OK != err_code)
    {
    ...
    session->message = /*(char*) ftpsrvmsg_tx_complete*/ "";
    return(FTPSRV_OK);
}
