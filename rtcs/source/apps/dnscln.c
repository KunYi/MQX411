...
* Function Prototypes
*******************************************************************/
static uint32_t dnscln_send (uint32_t sock, char *name_to_resolve, uint16_t type, char *tx_buffer, uint32_t *id_ptr);
static bool dnscln_domains_equal(char *req_domain, char *res_domain);
...
static uint32_t dnscln_send (uint32_t sock, char *name_to_resolve, uint16_t type, char *tx_buffer, uint32_t *id_ptr)
{
    ...
    message_head_ptr = (DNS_MESSAGE_HEADER_STRUCT *)tx_buffer;
    name_ptr  = &tx_buffer[sizeof(DNS_MESSAGE_HEADER_STRUCT)];
    ...
    *id_ptr = id;
    ...
    for (i=0; i<name_size; i++)
    {
        ...
        {
            mqx_htonc(name_ptr, name_to_resolve[i]);
            len++;
        }
        name_ptr++;
   }
...
*************************************************************************/
static DNSCLN_RECORD_STRUCT *dnscln_get_response(uint32_t  sock, char *name_to_resolve, uint16_t type, char *rx_buffer, uint32_t req_id)
{
    DNS_MESSAGE_HEADER_STRUCT_PTR       message_head_ptr;
    ...
    uint32_t                            i;
    uint32_t                            offset;
    DNSCLN_RECORD_STRUCT                *record_list = NULL;
    DNSCLN_RECORD_STRUCT                *record_prev = NULL;
    DNSCLN_RECORD_STRUCT                *record = NULL;
    uint32_t                            name_size;
        ...
        if ( (id == req_id ) && (query_count==1) && answer_count)
        {
            ...
            if (dnscln_domains_equal(name_to_resolve, temp_ptr))
            {
                ...
                for (i=0; i<answer_count; i++)
                {
                    ...
                    if((temp_ptr - rx_buffer + offset + sizeof(DNS_RESPONSE_RR_MIDDLE_STRUCT) + rdlength)  > response_size)
                    {
                        /* Wrong packet*/
                        break;
                    }
                    ...
                    if (answer_type == type)
                    {
                        ...
                        if(record)
                        {
                            record->data = RTCS_mem_alloc(rdlength);
                            if(record->data)
                            {
                                record->data_length = rdlength;                     /* Length of DNS Resource Record data, in bytes.*/
                                _mem_copy(&answer_ptr[offset  + sizeof(DNS_RESPONSE_RR_MIDDLE_STRUCT)], record->data, rdlength); /* Pointer to DNS Resource Record data in network byte order.*/
...
DNSCLN_RECORD_STRUCT *DNSCLN_query(DNSCLN_PARAM_STRUCT *params)
{
    ...
    if (params &&  params->name_to_resolve)
    {
        ...
        buf = RTCS_mem_alloc_zero(DNS_MAX_UDP_MESSAGE_SIZE);
        if (buf)
        {
            ...
            if(RTCS_SOCKADDR_PORT(&remote_addr) == 0)
                RTCS_SOCKADDR_PORT(&remote_addr) = DNS_NAMESERVER_PORT; /* Set default DNS port.*/
                ...
                if(setsockopt(sock, SOL_UDP, RTCS_SO_UDP_NONBLOCK_TX, &option_udp_nonblock, sizeof(option_udp_nonblock))== RTCS_OK)
                {
                    ...
                    if ( error == RTCS_OK )
                    {
                        ...
                        if (error == RTCS_OK )
                        {
                            ...
                            if (error == RTCS_OK )
                            {
                                if (RTCS_selectset(&sock, 1, DNSCLN_TIMEOUT) == sock)
                                {
...
bool DNSCLN_get_dns_addr(_rtcs_if_handle ihandle /*Optional.*/, uint32_t n, sockaddr *dns_server)
{
    ...
    #if RTCSCFG_ENABLE_IP4 //TBD Multiple IPv6 DNS severs.
        if(LWDNS_server_ipaddr != 0)
        {
            if(n == 0)
            {
                ((sockaddr_in *)dns_server)->sin_family = AF_INET;
                ((sockaddr_in *)dns_server)->sin_addr.s_addr = LWDNS_server_ipaddr;
    ...
    #if RTCSCFG_ENABLE_IP6   
        for(if_n = 0; (((ihandle)?(ip_if = (IP_IF_PTR)ihandle ):(ip_if = ip6_if_get_by_n(if_n))) != NULL) ; if_n++) /* Run over all interfaces.*/
        {
            for(dns_n = 0; RTCS6_if_get_dns_addr((_rtcs_if_handle)ip_if, dns_n, &dns6_addr) == TRUE; dns_n++) /* Run over all interfaces.*/
            {
                if(n == 0)
                {
