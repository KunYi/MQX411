...
{ /* Body */
   ...
   mqx_htonc(header_ptr->OP,    DHCPOP_BOOTREQUEST);
   mqx_htonc(header_ptr->HTYPE, dhcp_ptr->HANDLE->DEV_TYPE);
   mqx_htonc(header_ptr->HLEN,  dhcp_ptr->HANDLE->DEV_ADDRLEN);
   mqx_htonc(header_ptr->HOPS,  0);
   /* Pick a random transaction ID */
   xid = RTCS_rand();
   mqx_htonl(header_ptr->XID, xid);

   /* mqx_htons(header_ptr->FLAGS, DHCPFLAG_BROADCAST); */
   mqx_htons(header_ptr->FLAGS, _DHCP_broadcast ? DHCPFLAG_BROADCAST : 0);
   
   if ( dhcp_ptr->STATE == DHCP_INFORMED ) {
      mqx_htonl(header_ptr->CIADDR, dhcp_ptr->DATA.ClientIPaddress);
   } else {
      mqx_htonl(header_ptr->CIADDR, INADDR_ANY);
   } /* Endif */
   mqx_htonl(header_ptr->YIADDR, INADDR_ANY);
   mqx_htonl(header_ptr->SIADDR, INADDR_ANY);
   mqx_htonl(header_ptr->GIADDR, INADDR_ANY);
...
{ /* Body */
   ...
   mqx_htonl(outp, DHCP_MAGIC);            outp += 4;
   mqx_htonc(outp, DHCPOPT_MSGTYPE);       outp++;
   mqx_htonc(outp, 1);                     outp++;
   dhcp_ptr->MSGTYPE_PTR = outp;
   if ( dhcp_ptr->STATE == DHCP_INFORMED ) {
      mqx_htonc(outp, DHCPTYPE_DHCPINFORM);
   } else  {
      mqx_htonc(outp, DHCPTYPE_DHCPDISCOVER);
   } /* Endif */
   outp++;
   /*
   ...
   */
   ...
   if (opt_ptr) {
      ...
      if ( mqx_ntohs(opt_ptr + 2) < DHCP_MIN_MESSAGE_SIZE ) {
         mqx_htonc(outp, DHCPOPT_MSGSIZE);       outp++;
         mqx_htonc(outp, 2);                     outp++;
         mqx_htons(outp, DHCP_MIN_MESSAGE_SIZE); outp += 2;
      } else  {
   ...
   */
   ...
   if (opt_ptr && (dhcp_ptr->STATE != DHCP_INFORMED)) {
      for ( i = 0; i < 6; i++ ) {
         mqx_htonc(outp, addr_buffer[i]);
         outp++;
      } /* Endfor */
   } /* Endif */

   /* Add the DHCPEND flag last */
   mqx_htonc(outp, DHCPOPT_END);
   outp++;
...
{  /* Body */
   ...
   for ( i = 0 ; i < 13 ; i++ ) {
         mqx_htonc(outp, 0); outp++;
      } /* Endfor */
   outp -= 13;
   mqx_htonc(dhcp_ptr->REQUEST_OPT_PTR, 0);
   ...
   if ( dhcp_ptr->STATE == DHCP_INITIALIZE ) {
      mqx_htonc(dhcp_ptr->MSGTYPE_PTR, DHCPTYPE_DHCPDISCOVER);
      dhcp_ptr->CURRENT_PACKET_SIZE = dhcp_ptr->TOTAL_PACKET_SIZE;
   } else  {
      mqx_htonc(dhcp_ptr->MSGTYPE_PTR, DHCPTYPE_DHCPREQUEST);
      if ( dhcp_ptr->STATE == DHCP_REQUESTING ||
           dhcp_ptr->STATE == DHCP_DECLINING  ||
           dhcp_ptr->STATE == DHCP_RELEASING ) {
         mqx_htonc(outp, DHCPOPT_SERVERID); outp++;
         mqx_htonc(outp, 4); outp++;
         if ( dhcp_ptr->STATE == DHCP_RELEASING ) {
            mqx_htonc(dhcp_ptr->MSGTYPE_PTR, DHCPTYPE_DHCPRELEASE);
            mqx_htonl(outp, dhcp_ptr->DATA.ServerIp); outp += 4;
         } else  {
            mqx_htonl(outp, dhcp_ptr->NEW_DATA.ServerIp); outp += 4;
            mqx_htonc(outp, DHCPOPT_ADDRESS); outp++;
            mqx_htonc(outp, 4); outp++;
            mqx_htonl(outp, dhcp_ptr->NEW_DATA.ClientIPaddress); outp += 4;
            if (dhcp_ptr->STATE == DHCP_DECLINING ) {
               mqx_htonl(header_ptr->CIADDR, INADDR_ANY);
               mqx_htonc(dhcp_ptr->MSGTYPE_PTR, DHCPTYPE_DHCPDECLINE);
            } else  {
      ...
      mqx_htonc(outp, DHCPOPT_END); outp++;
      dhcp_ptr->CURRENT_PACKET_SIZE = outp - dhcp_ptr->PACKET;
   }/* Endif */
...
{ /* Body */
   ...
   header_ptr = (DHCP_HEADER *)dhcp_ptr->PACKET;
   mqx_htons(header_ptr->SECS, dhcp_ptr->SECS);
