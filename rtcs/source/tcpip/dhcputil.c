...
{ /* Body */
   ...
   mqx_htonc(opt, opttype); opt++;
   mqx_htonc(opt, 1);       opt++;
   mqx_htonc(opt, optval);  opt++;
...
{ /* Body */
   ...
   mqx_htonc(opt, opttype); opt++;
   mqx_htonc(opt, (unsigned char)num_options); opt++;

   for ( i = 0; i < num_options; i++ ) {
      mqx_htonc(opt, options[i]); opt++;
   } /* Endfor */
