...
{ /* Body */
    ...
    bool             local_host_zero =0;
    bool             local_host_eq=0;
    bool             local_port_eq=0;
    bool             remote_port_zero=0;
    bool             local_result =1;
   ...
   */
   ...
   tcb->state        = LISTEN;
    ...
    if(req_ptr->AF == AF_INET)
    {
        ...
        {
            tcb->rcvmss = IP_MTU(tcb->local_host, remote_host_v4) - (uint16_t) (IP_HEADSIZE + TCP_HEADSIZE);
        } /* Endif */
    }
    #if RTCSCFG_ENABLE_IP6
    else
    ...
    if(req_ptr->AF == AF_INET6)
    {
        ...
        {
            tcb->rcvmss = IP6_MTU(&(tcb->local_ipv6_host),&remote_host_v6) - (uint16_t) (IP6_HEADSIZE + TCP_HEADSIZE);
        } /* Endif */
    }
#endif /* RTCSCFG_ENABLE_IP6 */
...
{ /* Body */
   ...
   uint32_t           seq,ack;
   uint16_t           urg;
   int32_t            result;     /* returned result */
   bool           seg_okay;
   /*IPv6*/
      ...
      case LISTEN:
         ...
         if ( (flags & SYN) != 0 ) {                         /* THIRD */
            ...
            if ( IN6_ARE_ADDR_EQUAL(&tcb->remote_ipv6_host, &in6addr_any) )
            {
                 ...
                 tcb->rcvmss = IP6_MTU(&(tcb->local_ipv6_host),&ipv6_source) - (uint16_t) (IP6_HEADSIZE + TCP_HEADSIZE);
            ...
            tcb->remote_port = mqx_ntohs(seg->source_port);
            /*
            ...
            */
            ...
            switch ( tcb->state ) {
               ...
               case FINWAIT_2:
                  ...
                  if ( result != RTCS_OK ) {
                     RTCS_log_error( ERROR_TCP, RTCSERR_TCP_TIMER_FAILURE,
                        (uint32_t)result, (uint32_t)tcb, 3 );
                  } /* Endif */
                  TCP_PP_DROP;
         ...
         if ( (flags & SYN) != 0 ) {                         /* THIRD */
            ...
            if ( tcb->remote_host == INADDR_ANY ) {
               ...
               tcb->rcvmss = IP_MTU(tcb->local_host, source)
                  - (uint16_t) (IP_HEADSIZE + TCP_HEADSIZE);
                if (tcb->sndmss > tcb->rcvmss)  {
                  tcb->sndmss = tcb->rcvmss;
               }
               tcb->sndmax = tcb->sndmss;
...
{ /* Body */
   ...
   in_family = mqx_ntohc((pcb)->NETWORK_LAYER);
