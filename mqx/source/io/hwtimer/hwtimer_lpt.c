 ...
 * \param lpt_id[in]    Determines LPT module( Always 0).
...
{
    LPTMR_MemMapPtr lpt_ptr = (LPTMR_MemMapPtr)hwtimer->ll_context[0];
    uint32_t clock_id = hwtimer->clock_id;
    static const uint32_t clock_inputs_ordered[] = {
        CM_CLOCK_SOURCE_MCGIR,
        CM_CLOCK_SOURCE_LPO,
        CM_CLOCK_SOURCE_ERCLK32K,
        CM_CLOCK_SOURCE_OSCER
    };
    static const uint8_t  clock_inputs_log[] = {0, 1, 2, 3};
    uint32_t i, prescaler, psr = 0, cmr = 0;
    uint64_t compare, tmp, diff, min = (uint64_t)-1;
    ...
    {
        if (clock_id == clock_inputs_ordered[i])
        {
            clock_source_valid = TRUE;

            tmp = (uint64_t)divider * 1000000;
            ...
            {
                min = diff;
                psr = LPTMR_PSR_PBYP_MASK | LPTMR_PSR_PCS(clock_inputs_log[i]);
            ...
            for (prescaler = 1; prescaler < 16; prescaler++)
            {
                ...
                {
                    min = diff;
                    psr = LPTMR_PSR_PRESCALE(prescaler) | LPTMR_PSR_PCS(clock_inputs_log[i]);
                    cmr = LPTMR_CMR_COMPARE(compare - 1);
                    if (0 == min)
                    {
                        break;
