    ...
    )
{
    ...
    */
    ...
    {
        /* Something is wrong there is some data transfers */
    ...
    /* The data transfer is done - the clock could be again driven automatically */
    esdhc_ptr->SYSCTL &= ~(SDHC_SYSCTL_PEREN_MASK | SDHC_SYSCTL_HCKEN_MASK | SDHC_SYSCTL_IPGEN_MASK);
    ...
    /* If the busy command is used or auto command CMD12 (has busy with response) - just force the clock the SD card to allow card finish the operation */
    if((xfertyp & (SDHC_XFERTYP_RSPTYP(3) | SDHC_XFERTYP_AC12EN_MASK)) == (SDHC_XFERTYP_RSPTYP(3) | SDHC_XFERTYP_AC12EN_MASK))
        esdhc_ptr->SYSCTL |= SDHC_SYSCTL_PEREN_MASK | SDHC_SYSCTL_HCKEN_MASK | SDHC_SYSCTL_IPGEN_MASK;
    ...
    switch (cmd)
    {
        ...
        case IO_IOCTL_FLUSH_OUTPUT:
            ...
            if(esdhc_ptr->PRSSTAT & SDHC_PRSSTAT_CINS_MASK)
            {
                ...
                /* Switch the clock settings back to the automatic gain off mode */
                esdhc_ptr->SYSCTL &= ~(SDHC_SYSCTL_PEREN_MASK | SDHC_SYSCTL_HCKEN_MASK | SDHC_SYSCTL_IPGEN_MASK);
            }
            break;
    ...
    )
{
    ...
    /* If the busy signal is still available - just force the clock the SD card to allow card finish the operation */
    if(!(esdhc_ptr->PRSSTAT & SDHC_PRSSTAT_DLSL(1)))
        esdhc_ptr->SYSCTL |= SDHC_SYSCTL_PEREN_MASK | SDHC_SYSCTL_HCKEN_MASK | SDHC_SYSCTL_IPGEN_MASK;
