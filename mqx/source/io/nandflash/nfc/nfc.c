...
uint32_t nfc_swap_addr1, nfc_swap_addr2;
...
{ /* Body */
   ...
   for (count1 = page_number; count1 < (page_number + page_count); count1++)
   {
     ...
     if(swap)
     {
       *(uint32_t*)&NFC_SRAM_B0_REG(nfc_ptr, (nfc_swap_addr2 * 8)) = 0xFFFF0000;
       nfc_ptr->SECSZ  += 8;
     }
...
uint32_t nfc_mark_block_as_bad
   (
      /* [IN] the NAND flash information */
      IO_NANDFLASH_STRUCT_PTR nandflash_ptr,
