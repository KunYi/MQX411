...
*
*END*----------------------------------------------------------------------*/
void *_bsp_get_cfm_address(unsigned char fstr_set)
{
    if ( fstr_set == FSTR_SET_1) {    
        return (void *)(&((VMCF51EM_STRUCT_PTR)BSP_IPSBAR)->FTSR1);
    } 
    /* FTSR_SET_2 */
    else {
        return (void *)(&((VMCF51EM_STRUCT_PTR)BSP_IPSBAR)->FTSR2);
    }        
}
...
{
   return (void *)(&((VMCF51EM_STRUCT_PTR)BSP_IPSBAR)->PMC);
}
...
void _mcf51_swap_flash_and_reset()
{
    ...
    int32_t (*RunInRAM)( VMCF51EM_RTC_STRUCT_PTR );
    ...
    *RunInRAM = (int32_t(*)( VMCF51EM_RTC_STRUCT_PTR ))ramcode_ptr;   
