...
uint32_t _29wsxxxJ_test_erase_cmd(void *, uint16_t);
uint32_t _29wsxxxJ_test_program_cmd(void *, uint16_t);
...
{ /* Body */
   ...
   if (flashx_ptr->WIDTH == 16) {
      ...
      base16_ptr = (uint16_t *)flashx_ptr->BASE_ADDR; 
...
{ /* Body */
   ...
   if (flashx_ptr->WIDTH == 16) {
      ...
      *( sect16_ptr )    = 0x0030;    
      ...
      result = _29wsxxxJ_test_erase_cmd(input_sect_ptr, (uint16_t)0xFFFF);  
   } 
   else {
      result = FLASH_UNSUPPORTED_BUS_SIZE;
   }
...
{ /* Body */
   ...
   if (flashx_ptr->WIDTH == 16) {
      ...
      *( sect16_ptr + 0x555 ) = 0x0010;    
      ...
      /* chip erase test */
      result = _29wsxxxJ_test_erase_cmd((unsigned char *)sect16_ptr, (uint16_t)0xFFFF);     
   }
   else {
      result = FLASH_UNSUPPORTED_BUS_SIZE; 
   }
...
*
*END*----------------------------------------------------------------------*/
uint32_t _29wsxxxJ_test_erase_cmd
   (
      /* [IN] address */
      void                *to_ptr,
      /* written data */
...
*
*END*----------------------------------------------------------------------*/
uint32_t _29wsxxxJ_test_program_cmd
   (
      /* [IN] address */
      void                *to_ptr,
      /* written data */
...
*
*END*----------------------------------------------------------------------*/
static bool _29wsxxxJ_test_lock_bits
    (
      /* [IN] the flash information structure */
      IO_FLASHX_STRUCT_PTR flashx_ptr
    ) 
{
    uint16_t *base_address_ptr = (uint16_t *)flashx_ptr->BASE_ADDR;
    ...
    for ( i = 0; flashx_ptr->HW_BLOCK[i].NUM_SECTORS != 0; i++ ) {
        ...
        sector_address_ptr = (uint16_t *)flashx_ptr->HW_BLOCK[i].START_ADDR; 
        ...
        for ( sector_count = 0; sector_count < flashx_ptr->HW_BLOCK[i].NUM_SECTORS; sector_count ++){
            ...
            *( sector_address_ptr) = 0x0;
            ...
            (unsigned char *)sector_address_ptr += (flashx_ptr->HW_BLOCK[i].SECTOR_SIZE); 
        }       
    }
    /* exit flash unlocked */
    return UNLOCKED;    
...
*
*END*----------------------------------------------------------------------*/
static uint32_t _29wsxxxJ_check_ID
    (
      /* [IN] the flash information structure */
      IO_FLASHX_STRUCT_PTR flashx_ptr
    ) 
