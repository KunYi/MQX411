...
uint32_t _vi2s_get_mclk_value(KSAI_INFO_STRUCT_PTR io_info_ptr);
void _vi2s_set_mclk_freq(KSAI_INFO_STRUCT_PTR io_info_ptr, uint32_t freq);
void _vi2s_set_clk_src_int(uint8_t hw_channel);
void _vi2s_set_clk_src_ext(uint8_t hw_channel);
void _vi2s_set_sai_div(uint8_t hw_channel, uint8_t value);
...
uint32_t _ki2s_get_mclk_value(KSAI_INFO_STRUCT_PTR io_info_ptr);
#endif // END - Platform specific macros

void _ksai_tx_irq_handler(void *);
void _ksai_rx_irq_handler(void *);
void _ksai_rx_tx_irq_handler(void *);

void _ksai_hw_init(KSAI_INIT_STRUCT_PTR, I2S_MemMapPtr, _mqx_int);
uint32_t _ksai_format_in_data(char *input, AUDIO_DATA_FORMAT_PTR format);
void _ksai_format_out_data(uint32_t input, char *output, AUDIO_DATA_FORMAT_PTR format);
uint32_t _ksai_sine_wave(uint32_t, uint32_t, int16_t **);

uint32_t _ki2s_find_mclk_freq_div(CM_CLOCK_SOURCE src, uint16_t *frct, uint16_t *div, uint32_t frequency);
uint32_t _ki2s_set_master_mode(KSAI_INFO_STRUCT_PTR io_info_ptr, uint8_t mode);
uint32_t _ki2s_set_slave_mode(KSAI_INFO_STRUCT_PTR io_info_ptr, uint8_t mode);
...
{   /* Body */
    ...
    ksai_info_ptr = (KSAI_INFO_STRUCT_PTR) _mem_alloc_system_zero((uint32_t) sizeof(KSAI_INFO_STRUCT));
    ...
    /* Buffers allocation */
    ksai_info_ptr->BUFFER.DATA = (uint32_t *)_mem_alloc_system_zero(buffer_size * sizeof(uint32_t));
...
)
{
    ...
    channel = ksai_info_ptr->HW_CHANNEL;    
    ...
    if (io_sai_dev_ptr->FLAGS & I2S_IO_WRITE)
    {
        ...
        /* Wait transmitter */
        while (i2s_ptr->TCSR & I2S_TCSR_TE_MASK){}
    ...
    if ((ksai_info_ptr->OLD_TX_ISR != ksai_info_ptr->OLD_RX_ISR) && (io_sai_dev_ptr->FLAGS & I2S_IO_READ))
    {
        ...
        /* Wait receiver */
        while (i2s_ptr->RCSR & I2S_RCSR_RE_MASK){}
...
)
{
    ...
    uint32_t                    i = 0;
    uint32_t                    space;
    uint8_t                     tx_channel = ksai_info_ptr->TX_CHANNEL;
    uint32_t                    read_ptr;
    uint32_t                    write_ptr;
    bool                    stop = FALSE;
...
) 
{
    ...
    uint32_t                    i = 0;
    uint32_t                    data_n;
    uint8_t                     rx_channel = ksai_info_ptr->RX_CHANNEL;
    uint32_t                    read_ptr;
    uint32_t                    write_ptr;
...
)
{
    ...
    switch (cmd)
    {
        ...
        case IO_IOCTL_I2S_SET_CLOCK_MODE:
            ...
            else /* if success, update CLOCK_MODE field accordingly */
            {
                /* Get the real clock mode has been set */
                bclk_mode = 0;
                (i2s_ptr->TCR2 & I2S_TCR2_BCS_MASK) ? (bclk_mode |= I2S_TX_BCLK_SWAPPED) : (bclk_mode |= I2S_TX_BCLK_NORMAL);
                (i2s_ptr->TCR2 & I2S_TCR2_SYNC_MASK) ? (bclk_mode |= I2S_TX_SYNCHRONOUS) : (bclk_mode |= I2S_TX_ASYNCHRONOUS);

                (i2s_ptr->RCR2 & I2S_RCR2_BCS_MASK) ? (bclk_mode |= I2S_RX_BCLK_SWAPPED) : (bclk_mode |= I2S_RX_BCLK_NORMAL);
                (i2s_ptr->RCR2 & I2S_RCR2_SYNC_MASK) ? (bclk_mode |= I2S_RX_SYNCHRONOUS) : (bclk_mode |= I2S_RX_ASYNCHRONOUS);
                /* update clock mode field */
                io_info_ptr->CLOCK_MODE = bclk_mode;
            }
...
)
{
    ...
    *output = (int16_t *) _mem_alloc(sizeof(int16_t) * samples);
    ...
    for(i = 0; i < samples; i++)
    {
        ...
        *((*output) + i) = (int16_t) (BIT16_MAX * (4*angle*(180 - angle))/(40500 - angle * (180 - angle)));
        if (over180)
        {
            *((*output) + i) = -(*((*output) + i));
        }
...
void _vi2s_set_mclk_freq(KSAI_INFO_STRUCT_PTR io_info_ptr, uint32_t freq)
{
    ...
    (void)      iter_mclk; /* suppress 'unused variable' warning */
    uint8_t     pll_div = 4;
    uint8_t     sai_div = 1;
...
)
{
    ...
    */
    if ((bclk_mode & I2S_TX_SYNCHRONOUS) && (i2s_ptr->RCR2 & I2S_RCR2_SYNC_MASK))
    {
        /* Invalid combination of bclk modes */
        return(I2S_ERROR_INVALID_PARAMETER);
    }
    ...
    */
    if ((bclk_mode & I2S_RX_SYNCHRONOUS) && (i2s_ptr->TCR2 & I2S_TCR2_SYNC_MASK))
    {
        /* Invalid combination of bclk modes */
        return(I2S_ERROR_INVALID_PARAMETER);
    }
