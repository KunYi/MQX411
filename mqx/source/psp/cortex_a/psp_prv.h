...
#define _PSP_SET_PC_OF_INTERRUPTED_TASK(stack_ptr, func) \
   _psp_set_pc_of_interrupted_task((PSP_BASIC_INT_FRAME_STRUCT_PTR)stack_ptr, \
   (void (_CODE_PTR_)(uint32_t))(func))

/* This macro obtains the address of the kernel data structure */
#define _GET_KERNEL_DATA(x)     x = _mqx_kernel_data

#define _SET_KERNEL_DATA(x)     _mqx_kernel_data = (struct kernel_data_struct *)(x)
...
void _psp_set_pc_of_interrupted_task(PSP_BASIC_INT_FRAME_STRUCT_PTR,void (_CODE_PTR_)(uint32_t));
