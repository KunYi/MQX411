...
/* Define a couple of macros to allow easy reading and writing to a 32-bit
 * address. */
#define REG_WRITE(val, addr) (*(volatile uint32_t*)(addr) = (val))
#define REG_READ(addr) (*(volatile uint32_t*)(addr))

#define REG_PTR(addr) ((volatile uint32_t*)(addr))

#define REG_CLR_BIT(addr, bit) \
    (REG_WRITE((REG_READ(addr) & (0xFFFFFFFF ^ (1 << bit))), addr))

#define REG_SET_BIT(addr, bit) \
    (REG_WRITE((REG_READ(addr) | (1 << bit)), addr))
