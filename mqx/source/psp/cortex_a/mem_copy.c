...
#error Small mem copy is not supported for NEON extension
#endif
...
{
    uint32_t dst_addr = (uint32_t)to_ptr;
    uint32_t src_addr = (uint32_t)from_ptr;

    int32_t dst_offset = dst_addr % 4;
    int32_t src_offset = src_addr % 4;

    int32_t offset = (4 + src_offset - dst_offset) % 4;
    int32_t len = (4 - dst_offset)%4;
    int32_t left = 0;

    if (number_of_bytes < 24 + len) {
        /*short copying, using memcpy directly*/
        memcpy(to_ptr, from_ptr, number_of_bytes);

    } else {
        /*copying the preamble which to keep the destination address to align with 4 bytes*/
        memcpy(to_ptr, from_ptr, len);

        src_addr = ((src_addr + len) >> 2) << 2;
        to_ptr = (char*)to_ptr + len;
        number_of_bytes -= len;

        /*according with the bias between src and dst offsets, select the method*/
        switch (offset) {
           case 0:
                left = neon_copy_offset_0(to_ptr, (void *)src_addr, number_of_bytes);
                break;
           case 1:
                left = neon_copy_offset_1(to_ptr, (void *)src_addr, number_of_bytes);
                break;
           case 2:
                left = neon_copy_offset_2(to_ptr, (void *)src_addr, number_of_bytes);
                break;
           case 3:
                left = neon_copy_offset_3(to_ptr, (void *)src_addr, number_of_bytes);
                break;
        }

        /*copying the epilogue to make ensure all data is copied*/
        to_ptr = (char*)to_ptr + (number_of_bytes - left);
        from_ptr = (char*)from_ptr + (len + number_of_bytes - left);
        memcpy(to_ptr, from_ptr, left);
    }
}

#elif MQXCFG_MEM_COPY
