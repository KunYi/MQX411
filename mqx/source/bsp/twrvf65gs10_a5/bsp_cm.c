...
)
{
    ...
    switch (clock_source)
    {
        case CM_CLOCK_SOURCE_CORE:
            if (clock_configuration == BSP_CLOCK_CONFIGURATION_FULL_SPEED)
                return BSP_CORE_CLOCK;
            else if (clock_configuration == BSP_CLOCK_CONFIGURATION_USE_FIRC)
                return BSP_CORE_CLOCK_FIRC;

        case CM_CLOCK_SOURCE_BUS:
            if (clock_configuration == BSP_CLOCK_CONFIGURATION_FULL_SPEED)
                return BSP_BUS_CLOCK;
            else if (clock_configuration == BSP_CLOCK_CONFIGURATION_USE_FIRC)
                return BSP_BUS_CLOCK_FIRC;

        case CM_CLOCK_SOURCE_IPG:
            if (clock_configuration == BSP_CLOCK_CONFIGURATION_FULL_SPEED)
                return BSP_IPG_CLOCK;
            else if (clock_configuration == BSP_CLOCK_CONFIGURATION_USE_FIRC)
                return BSP_IPG_CLOCK_FIRC;
