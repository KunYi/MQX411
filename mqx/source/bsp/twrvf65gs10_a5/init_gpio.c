...
)
{
    switch (dev_num)
    {
        case 0:
            ...
            /* TBD: if enable module clock should be done in io_init ??? */
            /*
           // QSPI enables module clock
            */
...
)
{
    ...
    switch (dev_num)
    {
        case 0:
            ...
            CCM_CCGR0 |= CCM_CCGR0_CG0(0x1);
            CCM_CCGR1 |= CCM_CCGR1_CG7(0x0);
            CCM_CSCDR2 |= CCM_CSCDR2_CAN0_EN_MASK;
            
            // CAN0_TX (PTB15)
            IOMUXC_RGPIO(37) = 0x103DE2;

            // CAN0_RX (PTB14)
            IOMUXC_RGPIO(36) = 0x103DE1;

            status = MQX_OK;
            break;
        case 1:
            // Enable FlexCAN1 clock
            CCM_CCGR9 |= CCM_CCGR9_CG4(0x1);
            CCM_CCGR1 |= CCM_CCGR1_CG7(0x0);
            CCM_CSCDR2 |= CCM_CSCDR2_CAN1_EN_MASK;
...

{
    ...
    CCM_CSCDR3 = (CCM_CSCDR3 & ~CCM_CSCDR3_NFC_PRE_DIV_MASK) | CCM_CSCDR3_NFC_PRE_DIV(1);
