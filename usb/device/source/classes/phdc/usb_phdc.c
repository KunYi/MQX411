 ...
 *****************************************************************************/
static uint8_t USB_PHDC_Other_Requests(USB_SETUP_STRUCT * setup_packet,
                                 uint8_t **data,
                                 uint32_t *size,
                                 void   *arg);
...
)
{
    ...
    error = _usb_device_init(USBCFG_DEFAULT_DEVICE_CONTROLLER,
        (_usb_device_handle *)&devicePtr->controller_handle,(uint8_t)((info->count) + 1));
    if(error != USB_OK)
    {
      goto error1;
    }
                ...
                (devicePtr->ep_data.count_rx < PHDC_RX_ENDPOINTS))
        {
                ...
                (devicePtr->ep_data.count_tx < PHDC_TX_ENDPOINTS))
          {
              ...
              devicePtr->ep_data.ep_tx[count].endpoint =
                    (uint8_t)info->ep[ep_count].ep_num;
              devicePtr->ep_data.ep_tx[count].type =
                    (uint8_t)info->ep[ep_count].type;
              devicePtr->ep_data.ep_tx[count].size =
                    (uint32_t)info->ep[ep_count].size;
