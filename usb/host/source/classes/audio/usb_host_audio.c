...
static void usb_class_audio_cntrl_callback(void *, void *, unsigned char *, uint32_t, USB_STATUS);
static USB_STATUS check_valid_fu(USB_AUDIO_CTRL_DESC_FU_PTR,uint8_t,uint8_t);
static USB_STATUS check_valid_ep(USB_AUDIO_STREAM_DESC_SPECIFIC_ISO_ENDP_PTR,uint8_t);
...
{ /* Body */
    ...
    usb_hostdev_tr_init(&tr, (tr_callback) callback, (void *) call_parm);
    tr.G.TX_BUFFER = (unsigned char *) buffer;
...
{ /* Body */
    ...
    wIndex = (uint16_t)(((if_ptr->it_desc->bTerminalID)<<8)|(if_ptr->AUDIO_G.IFNUM));
