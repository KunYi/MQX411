...
{ /* Body */
   ...
   if (!status) {
      ...
      if (if_ptr->C_BUF == NULL) {
         ...
         status = _usb_hostdev_get_buffer(if_ptr->G.dev_handle, wlength, (void **)&if_ptr->C_BUF);
   ...
   if (!status) {
      ...
      *(uint16_t*)req.WLENGTH = HOST_TO_LE_SHORT(wlength);
      status = _usb_hostdev_cntrl_request(if_ptr->G.dev_handle, &req, (unsigned char *) if_ptr->C_BUF,
...
{ /* Body */
    ...
    if (!status) {
        ...
        if (if_ptr->I_BUF == NULL) {
            /* Allocate new memory for control pipe request */
            status = _usb_hostdev_get_buffer(if_ptr->G.dev_handle, len, (void **) &if_ptr->I_BUF);
            if_ptr->I_BUF_SIZE = len;
        }
    }

    if (!status) {
        usb_hostdev_tr_init(&tr, usb_class_hub_int_callback, (void *) if_ptr);
