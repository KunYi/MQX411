...
{ /* Body */
   ...
   mem_ptr->offset = (~(uint32_t) mem_ptr->payload.data + 1) & (align - 1);
   if (mem_ptr->offset) {
...
{ /* Body */
   ...
   if ((dev_instance_ptr == NULL) || (dev_instance_ptr->address != 1)) {
      ...
      if (new_instance_ptr->address >= 127) {
         ...
         USB_unlock();
         USB_mem_free((void *)new_instance_ptr);
         #ifdef _HOST_DEBUG_
   ...
   **-----------------------------------------------------------*/
...
#ifdef __USB_OTG__
   _usb_otg_get_status((void *)usb_otg_state_struct_ptr,
      USB_OTG_STATE, &state);

   if (state < B_IDLE) {
      _usb_otg_set_status((void *)usb_otg_state_struct_ptr,
   ...
   status = _usb_host_ch9_get_descriptor((_usb_device_instance_handle)new_instance_ptr,
      ...
      (unsigned char *)&new_instance_ptr->dev_descriptor);
...
{ /* Body */
   ...
   tr_ptr = pipe_ptr->tr_list_ptr;
   _usb_host_close_pipe(handle, (void *)pipe_ptr);
...
{ /* Body */
   ...
   DEV_INSTANCE_PTR  *device_root = (DEV_INSTANCE_PTR *)&usb_host_ptr->DEVICE_LIST_PTR;
#ifdef __USB_OTG__
   uint32_t           state;
#endif
