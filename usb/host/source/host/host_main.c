...
{ /* Body */
   ...
   {
      error = host_if->HOST_PREINIT((_usb_host_handle *) &usb_host_ptr);
   }
...
static void _usb_host_open_pipe_cleanup(_usb_host_handle, PIPE_STRUCT_PTR, bool);
...
{ /* Body */
   ...
   *pipe_as_params = *pipe_init_params_ptr;
   ...
   pipe_as_params->G.PIPE_ID = (int16_t)i;
...
{ /* Body */
   ...
   if (pipe_ptr == NULL || pipe_ptr->DEV_INSTANCE == NULL)
   {
      #ifdef _HOST_DEBUG_
         DEBUG_LOG_TRACE("_usb_host_set_up_tr invalid pipe");
      #endif
   ...
   if (pipe_tr_ptr == NULL) {
      #ifdef _HOST_DEBUG_
         DEBUG_LOG_TRACE("_usb_host_set_up_tr NULL transaction");
      #endif
...
{ /* Body */
   ...
   uint32_t                    status = USB_log_error(__FILE__,__LINE__,USBERR_INVALID_PIPE_HANDLE);
