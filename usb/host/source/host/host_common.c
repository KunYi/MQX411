...
static uint32_t Class_Key_Generator = 0;
...
static USB_STATUS usb_hostdev_get_class(_usb_device_instance_handle, uint16_t, void **);
static uint32_t usb_hostdev_mask_class_etc(uint8_t *, uint8_t *);
static USB_STATUS  usb_hostdev_open_pipe(_usb_device_instance_handle, _usb_interface_descriptor_handle, uint32_t, uint8_t, _usb_pipe_handle *);
static bool  usb_host_driver_info_match(DEV_INSTANCE_PTR, INTERFACE_DESCRIPTOR_PTR, USB_HOST_DRIVER_INFO_PTR);
static void  usb_host_driver_info_scan(DEV_INSTANCE_PTR, INTERFACE_DESCRIPTOR_PTR, uint32_t);
static bool usb_host_driver_info_nonzero(USB_HOST_DRIVER_INFO_PTR);
...
{ /* Body */
   ...
   offset = *((unsigned char *) buffer_ptr - 1);
   /* Find device memory pointer. */
   dev_mem = (DEV_MEMORY_PTR) ((unsigned char *) buffer_ptr - offset - MEM_HEADER_LEN);
...
* Returned Value : uint32_t, depends on coded_key in the parameter
...
*
*END*--------------------------------------------------------------------*/
uint32_t  usb_host_class_intf_validate
   (
      /* [IN] class-interface handle */
      void    *class_intf_handle
   )
{ /* Body */
   ...
   if (cc_ptr->code_key != 0 ) {
      ...
      #endif
      return (uint32_t)(cc_ptr->code_key == gen_ptr->key_code);
   } else {
      #ifdef _HOST_DEBUG_
         DEBUG_LOG_TRACE("usb_host_class_intf_validate SUCCESSFUL");
      #endif
...
*
*END*--------------------------------------------------------------------*/
static bool  usb_host_driver_info_match
   (
      /* [IN] USB device */
      DEV_INSTANCE_PTR           dev_ptr,
...
*
*END*--------------------------------------------------------------------*/
static bool  usb_host_driver_info_nonzero
   (
      /* [IN] USB device */
      USB_HOST_DRIVER_INFO_PTR           info_ptr
   )
...
static USB_STATUS  usb_hostdev_open_pipe
   (
      ...
      _usb_pipe_handle          *pipe_handle_ptr
   )
{ /* Body */
   DESCRIPTOR_UNION         desc;
   DEV_INSTANCE_PTR         dev;
   ...
   pipe_init_params.G.MAX_PACKET_SIZE = (uint16_t)(SHORT_UNALIGNED_LE_TO_HOST(desc.ndpt->wMaxPacketSize) & PACKET_SIZE_MASK);
   ...
   pipe_init_params.G.ENDPOINT_NUMBER = (uint8_t)(desc.ndpt->bEndpointAddress & ENDPOINT_MASK);
   pipe_init_params.G.DIRECTION = (uint8_t)((desc.ndpt->bEndpointAddress & IN_ENDPOINT) ? USB_RECV : USB_SEND);
...
{ /* Body */
   ...
   if (usb_host_driver_info_nonzero(info_ptr)) {
      ...
      error = usb_hostdev_get_class (dev_handle,
         ...
         (void **)&class_ptr->class_intf_handle);
...
uint32_t _usb_hostdev_get_speed
   (
      /* [IN] USB device */
      _usb_device_instance_handle      dev_handle
   )
