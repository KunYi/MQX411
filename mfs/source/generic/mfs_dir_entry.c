...
MFS_DIR_ENTRY_PTR MFS_Create_directory_entry
    ...
    char        *filename_ptr,       /*[IN] pointer to the file's name */
    char            attribute,          /*[IN] attribute to be assigned to the file */
    uint32_t     *dir_cluster_ptr,    /*[IN/OUT] indicates cluster in which the entry was put.*/
    uint32_t     *dir_index_ptr,      /*[IN/OUT] index of the location of new file within the directory */
    uint32_t     *error_ptr           /*[IN/OUT] error_return_address */
    )
{
    ...
    */
    ...
    else if (MFS_is_read_only (drive_ptr))
    {
                ...
                *error_ptr = MFS_Add_cluster_to_chain(drive_ptr, entry_cluster, &entry_cluster);
                if ( *error_ptr == MFS_NO_ERROR )
                {
                    *error_ptr = MFS_Clear_cluster(drive_ptr, entry_cluster);
                    ...
                    dir_entry_ptr = (void *) drive_ptr->DIR_SECTOR_PTR;
...
MFS_DIR_ENTRY_PTR MFS_Create_entry_slave
    ...
    uint32_t              *cluster_ptr,   /*[OUT] cluster # in the directory where the entry was created */
    uint32_t              *index_ptr,     /*[OUT] index # in the directory where the entry was created*/
    _mfs_error_ptr           error_ptr,     /*[IN/OUT] error code is written to this address */
    bool                  overwrite      /*[IN]  if TRUE, we will overwrite an existing entry w/the same name */
    )
{
    MFS_DIR_ENTRY_PTR   dir_entry_ptr;
    char            *temp_dirname;
...
_mfs_error  MFS_Increment_dir_index
    (
    MFS_DRIVE_STRUCT_PTR    drive_ptr,  /*[IN] the drive on which to operate */
    uint32_t    *cluster_ptr,         /*[IN/OUT]  the initial/next cluster # */
    uint32_t    *index_ptr,           /*[IN/OUT]  the initial/next index */
    uint32_t    *prev_cluster_ptr     /*[IN/OUT]  the prev cluster # */
    )
{
    uint32_t        index;
    uint32_t        cluster;
