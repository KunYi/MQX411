...
#else
int32_t _io_part_mgr_install(int dev_fd, char *identifier, uint32_t sector_size);
int _io_part_mgr_init(void *init_data, void **dev_context);
int _io_part_mgr_deinit(void *dev_context);
int _io_part_mgr_open(void *dev_context, const char *open_name_ptr, int flags, void **fp_context);
int _io_part_mgr_close(void *dev_context, void *fp_context);
int _io_part_mgr_read(void *dev_context, void *fp_context, void *buf, size_t nbytes);
int _io_part_mgr_write(void *dev_context, void *fp_context, const void *data_ptr, size_t num);
off_t _io_part_mgr_lseek(void *dev_context, void *fp_context, off_t offset, int whence);
int _io_part_mgr_ioctl(void *dev_context, void *fp_context, unsigned long int request, va_list ap);
#endif
